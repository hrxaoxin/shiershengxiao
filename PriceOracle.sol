 ...
 *
 * 核心功能�?
 * 1. 代币USD价格管理：存储和更新代币相对USD的价�?
 * 2. ETH/BNB USD价格管理：存储和更新原生代币的USD价格
 * 3. 价值换算：代币数量 �?USDT数量的换�?
 ...
 *    - Owner 先提议新价格，等�?priceUpdateCooldown（默�?分钟）后执行
 *    - 价格变动不得超过 maxPriceChangePercent（默�?0%�?
 *    - 防止 Owner 瞬间大幅篡改价格
 * 2. 授权合约直接更新（updateTokenPrice / updatePrices）：
 *    - �?authorizer 授权的可信合约（�?Chainlink 集成或官方喂价机器人）直接设�?
 *    - 变动同样�?maxPriceChangePercent 限制
 * 3. DEX 自动抓取（fetchPriceFromDEX / fetchPriceFromAllDEX）：
 *    - 通过 UniswapV2 风格�?pair 获取现货价格
 *    - fetchPriceFromAllDEX 从多�?DEX 取平均，减少单个 DEX 被操纵的影响
 *    - 需�?autoPriceEnabled = true
 *
 * 价格有效性检查：
 * - priceValidityPeriod（默�?4小时）：价格更新后超过此时间视为失效
 * - isTokenPriceValid() / isETHPriceValid() / isPriceValid() 供外部检�?
 *
 * 价格历史记录�?
 * - 使用环形缓冲区（priceHistory / priceHistoryStartIndex），最�?MAX_HISTORY_LENGTH = 100 �?
 ...
 * - tokenContract：需正确设置以识别代�?
 * - usdtContract：需正确设置以识�?USDT
 *
 * 典型业务调用场景�?
 * - NFTUpdate.sol：升级费�?= tokenAmount × tokenPriceUSD（把代币换算成USD价值验证）
 * - NFTTrading.sol：可选择�?BNB 价格作为 NFT 挂牌价的参�?
 * - RewardManager.sol：用 USD 价值评估奖励金�?
 *
 * 安全考虑�?
 ...
 * 主网部署时建议结�?Chainlink 或多签验证机制以增强安全性�?
 */
contract PriceOracle is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
     ...
     * @dev 更新代币价格
     *
     * @param _tokenPriceUSD 新的代币价格（USD，精�?8位）
     */
     ...
     * @dev 更新ETH价格
     *
     * @param _ethPriceUSD 新的ETH价格（USD，精�?8位）
     */
     ...
     * 将代币数量转换为USDT数量
     *
     * @param tokenAmount 代币数量（精�?8位）
     * @return uint256 USDT数量（精�?位）
     *
     * 计算公式�?
     ...
     * 将USDT数量转换为代币数�?
     *
     * @param usdtAmount USDT数量（精�?位）
     * @return uint256 代币数量（精�?8位）
     *
     * 计算公式�?
