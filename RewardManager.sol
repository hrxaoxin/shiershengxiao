    ...
    IRewardManager
{
     ...
     * 
     * 【业务说明】
     * 用户权重是分红分配的核心依据，权重越高，分得的红利越多。
     * 权重计算采用分层累加模式：每张NFT根据其属性和等级贡献不同权重值。
     * 
     * 【权重规则】
     * - 普通属性（风、水、火）权重：1阶=1, 2阶=2, 3阶=6, 4阶=18, 5阶=66
     * - 稀有属性（光、暗）权重：1阶=10, 2阶=12, 3阶=16, 4阶=28, 5阶=76
     * 
     * 【计算流程】
     * 1. 遍历用户所有NFT
     * 2. 根据NFT的元素属性（风/水/火/光/暗）选择对应的权重表
     * 3. 根据NFT的等级获取对应权重值
     * 4. 累加所有NFT的权重得到用户总权重
     * 
     * 【设计考量】
     * 1. 合约所有者的权重单独存储在ownerWeight中，不参与普通计算
     * 2. 普通用户的权重直接从NFTData合约计算，确保数据一致性
     * 3. 光暗属性权重高于风水火，体现稀有属性的价值
     * 4. 高阶NFT权重增长呈非线性，鼓励玩家升级NFT
     * 
     * @param user 用户地址
