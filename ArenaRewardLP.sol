...
contract ArenaRewardLP is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function _addToRewardPool(uint256 amount, RewardType type_) internal {
        if (type_ == RewardType.LP) {
            ...
            require(newBalance >= lpRewardPoolBalance, "ArenaRewardLP: LP overflow");
            ...
            require(newBalance >= tokenRewardPoolBalance, "ArenaRewardLP: Token overflow");
            ...
            require(newBalance >= bnbRewardPoolBalance, "ArenaRewardLP: BNB overflow");
    ...
    function claimLPReward(uint256 seasonId) external nonReentrant whenNotPaused returns (uint256) {
        ...
        arenaRewardContract.markRewardClaimed(msg.sender, seasonId);
    ...
    function setRewardRate(uint256 _rewardRate) external onlyOwner {
        require(_rewardRate > 0 && _rewardRate <= maxRewardRate, "ArenaRewardLP: Invalid reward rate");
    ...
    function setMaxRewardRate(uint256 _maxRewardRate) external onlyOwner {
        require(_maxRewardRate >= rewardRate, "ArenaRewardLP: Max rate must be >= current rate");
    ...
    /**
     * @dev 计算并释放每日奖励（用于竞技场，从奖励池中按比例释放到可领取奖励）
     */
