 ...
 * 
 * 核心职责�? * 1. 赛季生命周期管理：创建、启动、结算赛�? * 2. 战斗挑战系统：PvE（模拟玩家）�?PvP（真实玩家）挑战
 * 3. NFT 质押管理：用户质�?NFT 用于战斗队伍
 * 4. 战斗队伍配置：设置和清除战斗队伍
 * 
 * 赛季系统�? * - 每个赛季持续一定时间（默认1天，可配置）
 * - 赛季结束时结算，分配奖励
 * - 支持赛季历史查询
 * 
 * 战斗系统�? * - PvE 模式：挑战模拟玩家（mock players），根据难度获得不同分数
 * - PvP 模式：挑战真实玩家，胜者获得积�? * - 每日挑战次数限制（默�?次），可通过代币充值增�? * - 战斗冷却时间（默�?0秒）
 * 
 * 分数计算�? * - 挑战胜利�?10~25分（根据对手分数差异�? * - 挑战失败�?5~10分（根据对手分数差异�? * - 平局：不�? * 
 * 与其他合约的交互�? * - ArenaBattle：执行实际战斗逻辑
 * - ArenaLeaderboard：更新排行榜数据
 * - ArenaPlayer：管理玩�?NFT 质押和挑战次�? * - ArenaReward：发放赛季奖�? * 
 * 安全机制�? * - ReentrancyGuard：防止重入攻�? * - Pausable：可暂停所有操�? * - 非流动性设计：战斗不转�?NFT 所有权
 * 
 * 权限控制�? * - onlyOwner：设置合约地址、配置参�? * - onlyAuthorized：启动赛季、结算赛�? * 
 * 注意：此合约�?ArenaRanking 的拆分版本，专门负责写入操作
 * 查询功能�?ArenaRankingQuery 合约提供
 */
contract ArenaRankingManager is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    /**
     * @dev 模拟玩家信息结构�?     * @param team 模拟玩家队伍�?个NFT ID�?     * @param score 模拟玩家分数
     * @param level 模拟玩家等级
     * @param growth 模拟玩家成长�?     * @param elementCounts 各属性数量统�?     */
     ...
     * @param isActive 是否活跃�?     * @param isSettled 是否已结�?     * @param rewardCalculated 奖励是否已计�?     * @param totalPlayers 赛季参与玩家�?     * @param rewardPool BNB奖励�?     * @param tokenRewardPool 代币奖励�?     * @param pendingRewards 待发放奖�?     */
    struct SeasonInfo {
        uint256 seasonId;
        uint256 startTime;
        uint256 endTime;
    ...
    function setArenaPlayerContract(address _arenaPlayerContract) external onlyOwnerOrAuthorizer {
    ...
    function setArenaBattleContract(address _arenaBattleContract) external onlyOwnerOrAuthorizer {
    ...
    /**
     * @dev 设置竞技场模�?     * @param mode 模式值：0 = 单人模式, 1 = 组队模式
     */
    ...
    /**
     * @dev 设置模式控制类型
     * @param controlType 控制类型�? = 禁用, 1 = 手动, 2 = 自动
     */
    ...
    /**
     * @dev 配置竞技场模式（批量设置�?     * @param controlType 控制类型�? = 禁用, 1 = 手动, 2 = 自动
     * @param preferredMode 偏好模式�? = 单人模式, 1 = 组队模式
     */
    ...
    /**
     * @dev 设置最大充值次数限�?     * @param _limit 最大充值次数（1-10�?     */
    ...
    /**
     * @dev 设置奖励类型
     * @param _rewardType 奖励类型�? = BNB, 1 = 代币
     */
    ...
    /**
     * @dev 挑战模拟玩家（PvE 模式�?     * @param playerTeam 玩家战斗队伍�?�?NFT ID�?     * @param mockIndex 模拟玩家索引�?-1000�?     * @return success 挑战是否成功
     */
    ...
    /**
     * @dev 挑战真实玩家（PvP 模式�?     * @param challengedPlayer 被挑战玩家地址
     * @param playerTeam 挑战者的战斗队伍�?�?NFT ID�?     * @return success 挑战是否成功
     */
    ...
    /**
     * @dev 内部函数：清除玩家赛季数据（新赛季开始时调用�?     * @param player 玩家地址
     */
    ...
    /**
     * @dev 检查并启动新赛季（公共接口�?     * 如果当前赛季已结束，自动启动新赛�?     */
    ...
    /**
     * @dev 内部函数：清理旧赛季数据
     * 保留最近的 MAX_SEASONS_TO_KEEP 个赛季数据，删除更早的赛�?     */
    ...
    /**
     * @dev 内部函数：计算赛季奖�?     * @param seasonId 赛季 ID
     */
    ...
    /**
     * @dev 获取当前赛季奖励�?     * @return 当前赛季的奖励池金额
     */
    ...
    /**
     * @dev 质押 NFT
     * @param tokenIds 要质押的 NFT ID 数组�?-6 个）
     */
    ...
    /**
     * @dev 解除质押 NFT
     * @param tokenIds 要解除质押的 NFT ID 数组�?-6 个）
     */
    ...
    /**
     * @dev 设置战斗队伍（动态数组版本）
     * @param tokenIds 战斗队伍�?NFT ID 数组（必须恰�?6 个）
     */
    ...
    /**
     * @dev 设置战斗队伍（固定数组版本）
     * @param tokenIds 战斗队伍�?NFT ID 数组�? 个）
     */
    ...
    /**
     * @dev 内部函数：设置战斗队�?     * @param tokenIds 战斗队伍�?NFT ID 数组�? 个）
     */
